...
cmap_impl_create(uint32_t mask) {
    ...
    impl = (struct cmap_impl*)xzalloc_cacheline(sizeof *impl + (mask + 1) * sizeof *impl->buckets);
    impl->n = 0;
...
bool cmap_is_empty(const struct cmap* cmap) {
...
cmap_find(const struct cmap* cmap, uint32_t hash) {
    ...
    return cmap_find__(&impl->buckets[h1 & impl->mask], &impl->buckets[h2 & impl->mask], hash);
}
...
void cmap_cursor_advance(struct cmap_cursor* cursor) {
