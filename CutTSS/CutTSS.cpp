...
int CutTSS::ClassifyAPacket(const Packet& packet) {
    ...
    uint64_t Query = 0;
    if (nodeSet[0])
        matchPri = trieLookup(packet, nodeSet[0], 3, Query);
    if (nodeSet[1] && maxPri[1] > matchPri)
        matchPri = max(matchPri, trieLookup(packet, nodeSet[1], 1, Query));
    ...
    case 3: {
        while (node && !node->isLeaf) {
            ...
            cchild = ((delete_rule.range[0][LowDim] >> numbit1) & node->andbit[0]) *
                         node->ncuts[1] +
                     ((delete_rule.range[1][LowDim] >> numbit2) & node->andbit[1]);
            node = node->children[cchild];
        }
    }
    default:
